...
export class Handler {
    ...
    constructor(client: Client, options: Options) {
        ...
        this.client.on('interactionCreate', async(interaction) => {
            if(interaction.isCommand()) {
                ...
                if(!command) return console.log(`[DJS-Handler] Slash Command with name '${interaction.command.name}' isn't found!`);
    ...
    loadCommands(): Promise<boolean> {
        return new Promise(async (res, rej) => {
            const commandFiles = await glob(this.options.commandsDir + this.options.searchPattern);
            if(!commandFiles.length) return rej(`[DJS-Handler] No Commands Found.`);
    ...
    loadSlashCommands(): Promise<boolean> {
        return new Promise(async (res, rej) => {
            const commandFiles = await glob(this.options.slashCommandsDir + this.options.searchPattern);
            if(!commandFiles.length) return rej(`[DJS-Handler] No Slash Commands Found.`);
    ...
    loadEvents(): Promise<boolean> {
        return new Promise(async (res, rej) => {
            const eventFiles = await glob(this.options.eventsDir + this.options.searchPattern);
            ...
            eventFiles.forEach((val) => {
                ...
                (file.emitter || this.client).on(file.name, (...args) => file.run(this.client, this,...args));
